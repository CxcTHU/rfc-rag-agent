...
- `AGENT.MD` 的“当前推荐的第一步”仍是阶段 0 初始化任务；实际阶段 0 已完成，后续应以本进度文档和阶段索引为准，进入阶段 1。
- `AGENT.MD` 中检索策略部分有一处阶段表述需要后续校准：里程碑要求阶段 1 先做关键词检索、阶段 2 再做向量检索。
