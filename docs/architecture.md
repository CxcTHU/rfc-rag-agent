...
## Phase 68：单一生产 Agent 架构

Phase 68 把在线 Agent 收敛为唯一生产模式 `tool_calling_agent`：
...
API request 的 `mode` 只允许省略或为 `tool_calling_agent`；退休值
`react_agent`、`langgraph_agent`、`agentic`、`default` 返回 422，不再静默映射到当前链路。
响应、meta/chitchat/follow-up 短路与 SSE metadata 的 `mode` 均统一为
`tool_calling_agent`。

已删除的在线历史实现与专属资产：

- default `AgentService` 执行器（`service.py` 只保留当前运行时共用的 `AgentQueryResult` 数据合同）；
- `ReActAgentService`、ReAct action/planner 与 adaptive ReAct 映射；
- `LangGraphAgentService`、`graph_builder/nodes/state/checkpointer`；
- `app/services/agentic/` 的早期 LangGraph 图；
- 旧 Agent `memory_context` 及上述路径的专属测试、对比评测和 Judge 脚本；
- `langgraph`、`langgraph-checkpoint-redis` 依赖与 checkpoint 专属配置。

保留边界：`legacy_toolbox.py` 仍是当前 `AgentToolbox` 的实现基座，四个生产工具
`hybrid_search_knowledge`、`search_tables`、`search_figures`、
`analyze_user_image` 均通过 typed registry adapter 调用它；不得把文件名中的
“legacy”误判为未使用代码。`conversation/session_memory.py`、Redis query/evidence cache、
rate limiting 与 layered cache 都属于当前链路，也继续保留。

回滚只通过 Git 恢复 Phase 68 前的源码和依赖，再按正常部署流程重建；生产配置中没有
用于重新激活退休 Agent 的 mode 或 feature flag。下文 Phase 67 及更早章节中的旧模式说明
只记录当时架构，不构成当前在线兼容承诺。
...
Historical record: Phase 52 added a unified short-term memory context for LangGraph planning. Phase 68 removed this old Agent `memory_context` and the LangGraph planner path. The current conversation memory implementation in `app/services/conversation/session_memory.py` remains separate and active.
...
Historical record: Phase 51 did not change the then-current default runtime chain. It aligned LangGraph naming and added an evaluation layer that was retired by Phase 68:

```text
/agent/query mode="langgraph_agent" (retired in Phase 68)
-> LangGraphAgentService (retired in Phase 68)
-> planner_node
-> AgentToolbox nodes
-> answer_with_citations
```
...
The historical evaluation layer was `scripts/evaluate_phase51_performance.py`. It compared `/chat` Brain baseline, explicit Agent modes, and pgvector HNSW vs FAISS fallback through script-local switches. Phase 68 deletes that dedicated LangGraph performance evaluator.

## Phase 50 Planner Fast Model Addendum

Historical record: LangGraph Agent then supported an optional two-tier model pattern. Phase 68 removed this online pattern and its planner-provider injection:
...
Historically, `latency_trace.planner_model` recorded either `deterministic` or `provider/model`, and `planner_latency_ms` recorded planner LLM call duration. Phase 68 removes the online LangGraph planner configuration; remaining planner-named settings are legacy aliases used only by the current tool-calling runtime identity fallback.

## Phase 50 Architecture Delta: LangGraph Agent And Redis Cache Layer

Historical Phase 50 added a LangGraph comparison path without external data sources. Phase 68 removes that path and the former ReAct fallback; only `tool_calling_agent` remains online.
...
The historical graph layer was orchestration only. Retrieval, figure search, table search, uploaded-image analysis, and cited answer generation still lived behind `AgentToolbox`; Phase 68 preserves those toolbox capabilities under the current `tool_calling_agent` path while deleting the graph callers.
...
Runtime-only objects such as `AgentToolbox` and SSE `event_sink` were injected into LangGraph nodes through `ContextVar`, not stored in checkpoint state. Phase 68 deletes this checkpoint path.

Historical SSE compatibility was preserved while LangGraph existed. Phase 68 removes `langgraph_agent`; current `/agent/query/stream` events are emitted by `tool_calling_agent` only.

Deployment boundary after Phase 68: Redis remains for current cache/rate-limit uses. Redis-backed LangGraph checkpoint persistence and `MemorySaver` fallback are no longer production contracts.
...
Automatic `enrich_agent_response_with_figure_evidence()` is gated by `ENABLE_AUTO_FIGURE_ENRICHMENT`, default `false`. Historical ReAct figure evidence had to come from the `search_figures` tool; after Phase 68, the active Agent figure path is `tool_calling_agent -> AgentToolbox.search_figures`. `/chat` remains unchanged.
...
阶段 38 当时仍保留 ReAct/default 对照路径；Phase 68 已删除这些在线路径。现在 request mode 只允许省略或 `tool_calling_agent`，provider capability 护栏继续生效。
...
新增 provider 协议结构位于 `app/services/generation/chat_model.py`：`ChatToolFunction`、`ChatToolDefinition`、`ChatToolCall`、`ToolCallingChatModelResult` 与 `generate_with_tools()`。旧 `generate()` / `stream_generate()` 行为保持兼容，`/chat` 不被替换；历史 default Agent 与 `react_agent` 已在 Phase 68 删除。
...
该架构与阶段 34 的 tiered provider 设计存在历史取舍：当时 `react_agent` 可以使用 Flash planner + V4-Pro answer，而 `tool_calling_agent` 的 `LLM(messages, tools)` 同时承担工具决策和最终回答。Phase 68 删除 ReAct 路径后，这个差异只保留为历史质量/延迟背景。
...
-> AgentService / ReActAgentService / Agentic RAG (historical, retired in Phase 68)
...
- Historical `ReActAgentService.__init__(planner_chat_provider=None)`：缺省 None 时保持原 elif 短路 + chat_model_provider 兼容路径（deterministic 测试、agentic、default 不受影响）；不为 None 时禁用 elif 短路，让 LLM 每轮自主决策。Phase 68 已删除该服务。
...
-> BrainService or ReActAgentService (historical; ReAct retired in Phase 68)
...
Historical record: 阶段 32 改动的是 `/agent/query` 的 Agent 编排层和 `/agent/query/stream` 的可观测输出层，不改变 `/chat` 默认 RAG 问答链路。当时新的 `react_agent` 路径让模型在受控 action schema 中选择下一步，但真正的工具执行仍由后端 `AgentToolbox` 负责。Phase 68 已删除该在线路径。

```text
POST /agent/query or /agent/query/stream
-> chitchat short-circuit
-> mode == react_agent (retired in Phase 68)
-> ReActAgentService (retired in Phase 68)
...
Historical `app/services/agent/react_actions.py` defined the Phase 32 action contract：只允许 `search_knowledge`、`rewrite_query`、`answer_with_citations`、`refuse`、`final_answer`。Phase 68 删除该模块；当前只保留中性的 figure-intent helper in `intent_router.py`。

Historical `app/services/agent/react_service.py` implemented the ReAct loop。Phase 68 删除该服务和专属测试；当前循环边界由 `ToolCallingAgentService -> RunCoordinator -> ToolExecutor` 承担。
...
历史阶段 32 的旧静态前端曾提交 ReAct mode 并保留 default/agentic 对照。Phase 68 已移除该请求字段和全部退休在线模式；这段只记录当时 UI 演进。

阶段 32 评测曾新增 `scripts/evaluate_stage32_react_agent.py`。它使用 in-memory SQLite fixture、deterministic embedding/chat，并显式关闭 reranking provider，对照 `default`、`agentic_langgraph`、`react_agent`。Phase 68 删除该专属评测脚本；历史 CSV 仍只是阶段记录，不是当前测试入口。
...
-> stream_agent_query_events()
   -> detect_chitchat()
   -> historical default AgentService 或 agentic LangGraph (retired in Phase 68)
...
`chainlit_app.py` 复用阶段 25 `/agent/query/stream` 背后的 service 层函数，而不是对本机 FastAPI 发 HTTP 请求。这样可以避免容器内“服务调用自己”的端口和启动顺序问题。自动模式路由是历史行为；Phase 68 后 Agent 只保留 `tool_calling_agent`。
...
-> detect_chitchat(question)
   ...
   -> 未命中：historical classify_query_complexity() (retired in Phase 68)
-> historical default AgentService 或 agentic graph (retired in Phase 68)
...
- Historical `/agent/query` 与 `/agent/query/stream` 都在 `classify_query_complexity()` 之前调用 `detect_chitchat()`，保证 default 和 agentic 路径都不会收到闲聊问题。Phase 68 已删除旧复杂度分流模块。
...
-> historical default AgentService.query(history=...) 或 run_agentic_rag(history=...) (retired in Phase 68)
...
```text
POST /agent/query (historical Phase 23 behavior, retired in Phase 68)
-> request.mode 显式存在？
   yes: 尊重 mode=default / mode=agentic
   no: classify_query_complexity(question) (retired in Phase 68)
...
```text
app/services/agent/routing.py (retired in Phase 68)
  classify_query_complexity(question)
  -> QueryComplexityResult(complexity, score, reasons, signals)
```

历史规则只读取问题文本，不调用 LLM。主要信号包括问题长度、子句数、对比/流程/多方面关键词、跨证据/改写倾向，以及 `search + compare/explain/analyze` 组合。Phase 68 已删除该自动分流在线入口。
...
-> default AgentService vs agentic LangGraph (historical, retired in Phase 68)
...
Historically, this kept Phase 37 inside the existing provider topology: no LangGraph, no checkpointing framework, no write tools, no default provider replacement, and no default routing switch. Phase 68 supersedes the ReAct comparison tradeoff by making `tool_calling_agent` the only production Agent chain.
...
Table extraction uses PyMuPDF `page.find_tables()` and stores extracted Markdown as `chunk_type="table"`. User uploads are validated with Pillow and saved under `data/user_uploads/`; the image-analysis capability originated in the ReAct path but is retained through current `AgentToolbox.analyze_user_image` under `tool_calling_agent`. It calls the configured vision provider through the existing provider abstraction, records `vision_provider` / `vision_model` in `image_analysis`, and refuses deterministic test vision as non-real image understanding. Citation location is best-effort: exact bbox, partial bbox, page-only, or none. Feedback export is local and sanitized before writing `data/evaluation/phase47_user_feedback_eval.csv`.
...
2. Historical LangGraph checkpointer -> RedisSaver on Redis Stack, fallback to MemorySaver (retired in Phase 68)
3. Rate Limiting -> Redis ZSET sliding window for Agent endpoints, default disabled
```

`docker-compose.dev.yml` and `docker-compose.prod.yml` use `redis/redis-stack-server:latest` so RedisJSON and RediSearch are available. Historical LangGraph checkpoint state was JSON-native dict/list data; Phase 68 removed that checkpointing runtime. The old answer-level Semantic Cache runtime has been removed; current cache reuse is evidence-path only.
