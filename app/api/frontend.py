...
QUALITY_SUMMARY_PATH = ROOT_DIR / "data" / "evaluation" / "stage29_quality_summary.csv"
...
def quality_report_data() -> JSONResponse:
    """只读返回阶段 29 质量门槛汇总（来自本地脱敏 CSV，不触发真实 API）。"""
...
def quality_report_export_csv() -> Response:
    ...
    return FileResponse(
        ...
        media_type="text/csv",
        filename="stage29_quality_summary.csv",
    )
