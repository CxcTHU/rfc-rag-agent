...
- The current cloud fix preserves the same providers and models while routing provider HTTPS traffic through a GPU egress SSH tunnel: `172.18.0.1:18443 -> api.deepseek.com:443` and `172.18.0.1:18444 -> llmapi.paratera.com:443`, managed by `rfc-provider-tunnel.service` and activated in Docker by `docker-compose.provider-tunnel.yml`.
- After the provider egress tunnel, minimal cloud benchmarks returned to local-like timing (`deepseek-v4-pro` chat about `3.4s`, tool-call request about `3.2s`, GLM embedding about `0.3s`). A full authenticated `/agent/query` for `堆石混凝土的优势` completed in about `27s` instead of about `238s`.
