...
## Phase 68 Final Agent Code Cleanup

The public production Agent has one mode: `tool_calling_agent`. Both
`POST /agent/query` and `POST /agent/query/stream` use the same runtime:
...
-> RunCoordinator
-> ToolExecutor
-> tool_registry adapters
-> AgentToolbox
```

Requests may omit `mode` or send the exact value `tool_calling_agent`.
`react_agent`, `langgraph_agent`, `agentic`, and `default` are no longer online
Agent modes and are rejected with HTTP 422. The retired `AgentService`,
`ReActAgentService`, `LangGraphAgentService`, both LangGraph graph packages,
the old Agent `memory_context`, their exclusive tests/evaluators, and the
LangGraph dependencies were removed.

`legacy_toolbox.py` is intentionally retained: the current `AgentToolbox`
inherits its implementation, including `hybrid_search_knowledge`,
`search_tables`, `search_figures`, and `analyze_user_image`. Current
conversation summaries and Redis-backed runtime caches are also retained; they
are not part of the retired LangGraph memory/checkpoint path.

Rollback is source rollback through Git to the commit before Phase 68, followed
by the normal dependency install and deployment process. There is no runtime
mode flag that reactivates a retired Agent implementation. Sections below that
describe earlier explicit ReAct/LangGraph/default modes are historical records,
not current API or rollback commitments.
...
Historical Phase 52 unified Phase 43 `SessionMemory` and Phase 51 LangGraph prior evidence into a short-lived `AgentMemoryContext`. The semantic upgrade added `MemoryIntentClassifier` (LLM JSON classifier with deterministic fallback), `PriorEvidenceRelevanceGate` (embedding similarity instead of `source_count >= 3`), and `MemoryItem(text, turn_index, importance)` recency decay for session memory. Phase 68 removed this old Agent `memory_context`; the current active session memory remains `app/services/conversation/session_memory.py`.

Main historical additions: `app/services/agent/memory_context.py`, `tests/test_phase52_memory_intent_classifier.py`, `tests/test_phase52_prior_relevance_gate.py`, memory trace fields in `latency_trace`, `scripts/evaluate_phase52_memory.py`, `data/evaluation/phase52_memory_regression_cases.csv`, and `docs/stage52_agent_memory_context.md`. Phase 68 deletes the online code/tests/evaluators for that path. Long-term memory was represented only by disabled governance interfaces and a read-none/write-none/delete-noop provider stub. No external data source or default provider changed.
...
Historical Phase 51 record: starting from the merged Phase 50 baseline, the LangGraph planning node was renamed from the old `route_query_node` implementation to `planner_node`, matching its then-current responsibility of choosing the next ReAct action through deterministic rules or the optional `PLANNER_CHAT_MODEL_*` fast planner. Phase 68 removed this online Agent path; final cited answers now use the single `tool_calling_agent` production chain.

At that time, planner config was blank by default, so LangGraph used `DeterministicReActPlanner` with zero provider calls and fell back to deterministic routing on invalid planner JSON or provider errors. This is not a current compatibility promise: Phase 68 rejects retired Agent modes at the API boundary.
...
Historical Phase 50 note: LangGraph, ReAct, and default modes were then available for opt-in comparison. Phase 68 removes all three online paths; this paragraph records the former experiment and is not a current rollback contract. Redis-backed caching and rate limiting remain current independently of LangGraph.
...
Phase 50 started from the merged Phase 49 baseline `main / origin/main -> 0671a31b Merge pull request #14 from CxcTHU/codex/phase-49-local-postgresql-cloud-sync`; `phase-49-complete` still points to `a044ce0c Complete phase 49 local PostgreSQL cloud sync` and was not moved. It historically introduced a LangGraph comparison route; Phase 68 deleted that route and its former ReAct rollback path.

Historical changes included Redis 7, query-embedding caching, a LangGraph state/checkpointer experiment, and deterministic ReAct-vs-LangGraph outputs. Phase 68 removed the graph/checkpointer/API experiment and its exclusive evaluators; Redis query caching remains part of the current runtime.
...
Boundary: Redis remains optional for current query/evidence cache use. Historical LangGraph checkpointing fallback to `MemorySaver` was removed in Phase 68 along with the graph/checkpointer runtime. No external data source was added, and no API key, bearer token, provider raw response, or restricted full text is written to Git/docs/tests/Obsidian. Phase 50 was stopped before `git add`, commit, tag, push, or PR until user human verification.
...
Phase 46 extension: figure retrieval became explicit and precision-first. The historical ReAct path used a read-only `search_figures(query, top_k=4)` tool over `image_description` chunks, with runtime image-quality checks, page metadata, caption propagation, and `MIN_IMAGE_RELEVANCE_SCORE=0.50`. Phase 68 removed the ReAct Agent implementation, but the production `AgentToolbox.search_figures` tool remains active under `tool_calling_agent`. Automatic `enrich_agent_response_with_figure_evidence()` remains disabled by default through `ENABLE_AUTO_FIGURE_ENRICHMENT=false`. The first 32-question deterministic fixture evaluation remains as historical unit coverage, and Phase 16-21 added a 100-question real-corpus evaluation set from actual image chunks/captions/page numbers. The real-corpus offline baseline produced `image_precision=0.9305`, `must_have_recall=1.0000`, `image_suppression=1.0000`, `topk_caption_match_rate=0.8800`, and `wrong_generic_curve_rate=0.0000`, so caption-weighted rerank and caption-enhanced embedding rebuild were not triggered.
...
Historical Phase 38 result: omitted mode entered `tool_calling_agent`, while ReAct/default were still temporary comparison paths. Phase 68 removes those comparison paths; the cited `structured_final_answer` six-metric Judge result remains historical quality evidence.
...
Historical Phase 37 record: it introduced `mode="tool_calling_agent"` as a parallel OpenAI-compatible `tools/tool_calls` loop beside the then-existing ReAct/default routes. Phase 68 promotes this to the only production Agent mode and removes the retired comparison routes.
...
Historical comparison: deterministic `tool_calling_agent` had `errors=0`, `same_refusal_as_react=8/8`, and `same_top_source_as_react=6/8`. Real-provider `--execute --limit 8` had `react_agent` errors=0, same_refusal=8/8, same_top_source=8/8; `tool_calling_agent` errors=0, same_refusal=8/8, same_top_source=7/8, avg final latency 13.5s vs ReAct 28.0s. The old recommendation to keep it as a parallel review mode is superseded by Phase 68.
...
Historical verification at that phase recorded `694 passed` and a ReAct endpoint smoke. That endpoint no longer exists after Phase 68; current verification must use the mode-less `tool_calling_agent` request.
...
- 历史新增 `app/services/agent/react_actions.py` 与 `app/services/agent/react_service.py`，让 `react_agent` 在受控 action schema 中选择 `search_knowledge`、`rewrite_query`、`answer_with_citations`、`refuse` 或 `final_answer`；Phase 68 已删除这些在线实现。
- 当时 ReAct 检索和回答仍复用 `AgentToolbox` 与 Brain 链路，不绕过 citation、sources、evidence confidence、responsibility_gate 或 refusal 约束；当前这些约束由 `tool_calling_agent` 生产链路承载。
- 历史阶段 32 曾提供 ReAct/default/agentic 对照入口；Phase 68 已删除这些在线入口与实现，`/chat` 仍是独立链路。
- `/agent/query/stream` 在保留 `token`、`metadata`、`done`、`error` 的基础上新增 `agent_step`、`tool_call_start`、`tool_call_result`。
- 历史旧前端 Agent 面板默认走 `react_agent`，运行中只显示简洁中文状态；Phase 68 后当前前端不再发送退休 mode。
- 历史新增 `scripts/evaluate_stage32_react_agent.py`，使用 deterministic fixture 对照 `default`、`agentic_langgraph`、`react_agent`，不依赖真实 provider；该专属评测脚本已在 Phase 68 删除。
...
- **历史闲聊短路前置**：新增 `app/services/agent/chitchat.py`，当时在旧自动分流 `classify_query_complexity()` 之前统一识别 `greeting`、`thanks`、`goodbye`、`acknowledgment`、`help` 五类社交意图，命中后直接返回友好回复，不调用 LLM 和检索。Phase 68 已删除旧复杂度分流模块。
- **历史 AgentService 边界收窄**：从 `AgentService.detect_intent()` 移除已提升的 greeting 分支，当时 default AgentService 继续只负责 answer/search/list_sources/get_source_detail 等 RAG/资料查询意图；Phase 68 已删除 default AgentService 在线路径。
...
- **历史 SSE Agent 端点**：新增 `POST /agent/query/stream`，返回 `text/event-stream`，事件格式稳定为 `token` / `metadata` / `done` / `error`；当时 default 和 agentic 路径同步完成 retrieve/grade/rewrite，generate 输出阶段通过队列实时发送 token。Phase 68 后该端点只由 `tool_calling_agent` 生产链路承载。
...
- **历史问题复杂度路由**：曾新增 `app/services/agent/routing.py` 的 `classify_query_complexity()`，规则式输出 `simple` / `complex`、分数、判断依据和命中信号；Phase 68 已删除该旧 default/agentic 自动分流模块。
- **历史 API 自动分流**：当时 `POST /agent/query` 未传 `mode` 会在 default `AgentService` 与 agentic LangGraph 间自动分流，并保留显式 `mode=default` / `mode=agentic` 调试覆盖。Phase 68 已删除该在线分流与调试覆盖；未传 `mode` 进入 `tool_calling_agent`，退休 mode 返回 422。
...
- **历史 LangGraph 状态图**（`app/services/agentic/`）：当时用 LangGraph StateGraph 构建 agentic RAG 编排图，节点包裹现有 HybridSearchService / BrainService 核心能力；Phase 68 已删除该在线模块。
...
- **历史可配置 mode 接入**：`/agent/query` 曾新增 `mode="agentic"` 参数，不替换默认 `/chat` 或 Brain hybrid 链路；Phase 68 后该参数不再受理。
...
- `app/services/agentic/` 阶段 21 LangGraph agentic RAG 模块（state.py、nodes.py、graph.py）；Phase 68 已删除该模块。
...
- `app/services/agent/routing.py` 阶段 23 规则式 `classify_query_complexity()`（Phase 68 已删除）
...
- User images can be uploaded to `data/user_uploads/` and analyzed through the current `AgentToolbox.analyze_user_image` preflight/tool path. This capability originated in the ReAct phase, but Phase 68 keeps the tool implementation under `tool_calling_agent` and removes the ReAct service.
